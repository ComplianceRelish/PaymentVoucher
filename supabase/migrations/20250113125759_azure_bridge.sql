...
CREATE POLICY "account_heads_read"
  ON account_heads FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "account_heads_write"
  ON account_heads FOR INSERT
  TO authenticated
  WITH CHECK (is_admin(auth.uid()));

CREATE POLICY "account_heads_modify"
  ON account_heads FOR UPDATE
  TO authenticated
  USING (is_admin(auth.uid()));

CREATE POLICY "account_heads_delete"
  ON account_heads FOR DELETE
  TO authenticated
  USING (is_admin(auth.uid()));

CREATE POLICY "payment_vouchers_read"
  ON payment_vouchers FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "payment_vouchers_write"
...
CREATE POLICY "payment_vouchers_modify"
